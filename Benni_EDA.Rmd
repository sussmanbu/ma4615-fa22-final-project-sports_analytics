...
scoring_data <- data %>% 
  ...
  filter(total_games>58, mp>54000)
...
segment_model <- function(bb_player, stat){
  d1 <- offense_data %>% filter(player==bb_player)
  ...
  pace_length <- (breakpoint - lag(breakpoint))[2:(n+2)]
  weighted_pace <- weighted.mean(pace, pace_length)
  tibble(name = bb_player, team = team, total = total, total_games = total_games, slope  = slope, weighted_pace = weighted_pace, pace = pace, pace_length = pace_length)
}
```

```{r NBA_view_individual}
d1 <- scoring_data %>% filter(player_id=="lowryky01")
