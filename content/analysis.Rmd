...
ggplot(bottom_PRA_streaks_2022) +geom_col(aes(y=max_streak,x=player)) + ggtitle('worst PRA streaks')
ggplot(bottom_streaks_2022) +geom_col(aes(y=max_streak,x=player)) + ggtitle('worst pts streaks')

ggplot(best_streaks_2022,aes(max_streak))+stat_ecdf(geom = "step") + ggtitle('best pts streaks')
ggplot(best_streaks_2022,aes(max_streak))+geom_density() + ggtitle('best pts streaks')


ggplot(bottom_streaks_2022,aes(max_streak))+stat_ecdf(geom = "step") + ggtitle('worst pts streaks')
ggplot(bottom_streaks_2022,aes(max_streak))+geom_density() + ggtitle('worst pts streaks')
...
ggplot(all_data_mlb) +geom_freqpoly(aes(x=h_streak))  + ggtitle('Distribution of max streaks in MLB') 
...
ggplot(best_streaks_2022) +geom_col(aes(y=max_streak,x=player))+ ggtitle('players with mean best streaks in MLB') 
ggplot(bottom_streaks_2022) +geom_col(aes(y=max_streak,x=player)) +ggtitle('players with mean worst streaks in MLB') 



#cdf and pdf for  best player hits
ggplot(best_streaks_2022,aes(max_streak))+stat_ecdf(geom = "step") +ggtitle('CDF of max streaks in MLB') 
ggplot(best_streaks_2022,aes(max_streak))+geom_density()+ ggtitle('Density of max streaks in MLB') 
...
all_data_mlb_turner <-all_data_mlb %>% filter(player=='Trea Turner') %>% add_predictions(mod2, type = "response" )
ggplot(all_data_mlb_turner) +geom_point(aes(x=h,y=pred,color=pa),position='jitter')

scoring<-all_data_mlb %>% filter(player == 'Trea Turner') %>%select(change_net,game_date,h_streak)
ggplot(scoring, aes(x=as.Date(as.factor(game_date)), y=change_net, group=1)) +
geom_ribbon(aes(ymin=pmin(change_net,0), ymax=0), fill="red", col="red", alpha=0.5) +
geom_ribbon(aes(ymin=0, ymax=pmax(change_net,0)), fill="green", col="green", alpha=0.5) +
geom_point()+
geom_line(aes(y=0)) + ggtitle('TT Change') +
ylab('Points Deviated from Average') +
xlab('Date')
ggplot(scoring,aes(h_streak))+stat_ecdf(geom = "step") + ggtitle('TT Change') 
...
ylab('Points Deviated from Average') +
xlab('Date')
ggplot(scoring,aes(h_streak))+stat_ecdf(geom = "step") + ggtitle('YD Change') 
...
```{r comparison 1}
ggplot(all_data_mlb) +geom_freqpoly(aes(x=h_streak)) + ggtitle('Distribution of hits streaks') 
ggplot(all_data) +geom_freqpoly(aes(x=pts_streak)) + ggtitle('Distribution of points streaks')

summary(all_data$pts_streak)
summary(all_data_mlb$h_streak)
```

Next, lets look at the top performers in the MLB and the NBA. Austin Hays had the highest average streak length in the MLB, and RJ Barrett had the highest in the same category in the NBA. 
```{r comparison 2}
...
scoring<-all_data %>% filter(player == 'RJ Barrett') %>%select(change_net,game_date,pts_streak)
ggplot(scoring, aes(x=as.Date(as.factor(game_date)), y=change_net, group=1)) +
geom_ribbon(aes(ymin=pmin(change_net,0), ymax=0), fill="red", col="red", alpha=0.5) +
geom_ribbon(aes(ymin=0, ymax=pmax(change_net,0)), fill="green", col="green", alpha=0.5) +
geom_point()+
...
mod2 <-all_data_mlb %>% filter(player=='Austin Hays') %>% ungroup() %>% glm(formula=factor(h_direction)~pa+rbi+home_runs+so,family=binomial())
mod2$coefficients
exp(mod2$coefficients)
summary(mod2)
...
ggplot(all_data_mlb_hays) +geom_point(aes(x=h,y=pred,color=pa),position='jitter')
...
```{r comparison 3 between all players}
#model building by NBA player
