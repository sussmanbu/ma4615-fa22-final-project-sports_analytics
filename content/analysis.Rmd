...
Next, lets take a closer look at some individual players and see how they are performing in 2022
...
ggplot(bottom_streaks_2022,aes(max_streak))+stat_ecdf(geom = "step")
ggplot(bottom_streaks_2022,aes(max_streak))+geom_density()
...
xlab('Date')
ggplot(scoring,aes(pts_streak))+stat_ecdf(geom = "step")

scoring<-all_data %>% filter(player == 'RJ Barrett',season == '2022') %>%select(change_net,game_date)
ggplot(scoring, aes(x=as.Date(as.factor(game_date)), y=change_net, group=1)) +
geom_ribbon(aes(ymin=pmin(change_net,0), ymax=0), fill="red", col="red", alpha=0.5) +
geom_ribbon(aes(ymin=0, ymax=pmax(change_net,0)), fill="green", col="green", alpha=0.5) +
geom_point()+
...
scoring<-all_data %>% filter(player == 'Kevin Durant',season == '2022') %>%select(change_net,game_date)
ggplot(scoring, aes(x=as.Date(as.factor(game_date)), y=change_net, group=1)) +
geom_ribbon(aes(ymin=pmin(change_net,0), ymax=0), fill="red", col="red", alpha=0.5) +
geom_ribbon(aes(ymin=0, ymax=pmax(change_net,0)), fill="green", col="green", alpha=0.5) +
geom_point()+
...
scoring<-all_data %>% filter(player == 'Klay Thompson',season == '2022') %>%select(change_net,game_date)
ggplot(scoring, aes(x=as.Date(as.factor(game_date)), y=change_net, group=1)) +
geom_ribbon(aes(ymin=pmin(change_net,0), ymax=0), fill="red", col="red", alpha=0.5) +
geom_ribbon(aes(ymin=0, ymax=pmax(change_net,0)), fill="green", col="green", alpha=0.5) +
geom_point()+
...
ggplot(all_data_mlb) +geom_freqpoly(aes(x=h_streak))
...
#cdf and pdf for  best player points
ggplot(best_streaks_2022,aes(max_streak))+stat_ecdf(geom = "step")
ggplot(best_streaks_2022,aes(max_streak))+geom_density()


#model building by player
mod1 <-all_data_mlb %>%ungroup() %>% glm(formula=factor(h_direction)~pa+rbi+home_runs+so,family=binomial())
mod1$coefficients
exp(mod1$coefficients)
...
ylab('Points Deviated from Average') +
xlab('Date')
ggplot(scoring,aes(h_streak))+stat_ecdf(geom = "step")
...
geom_ribbon(aes(ymin=0, ymax=pmax(change_net,0)), fill="green", col="green", alpha=0.5) +
geom_point()+
geom_line(aes(y=0)) + ggtitle('AD Change') +
ylab('Points Deviated from Average') +
xlab('Date')
...
First, let's look at the comparisons in aggregate data in the NBA and MLB by points,PRA, and hits. As you can see, surprisingly, there is a very similar trend among each league, and the distributions match very closely
```{r comparison 1}
ggplot(all_data_mlb) +geom_freqpoly(aes(x=h_streak))
summary(all_data_mlb$h_streak)
ggplot(all_data) +geom_freqpoly(aes(x=pts_streak))
...
Next, lets look at the top performer in the MLB and the NBA
```{r comparison 2}
...
```
Unfortunately, we can't use quite the same visualizations for the model, since the range of the number of hits is greatly constrained compared to the offensive rating of a player. In a further analysis, maybe batting average would provide the range necessary. 