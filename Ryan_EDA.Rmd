...
ggplot(sim, aes(epm, per)) + geom_point() +
  ...
  labs(x = "Effective Plus Minus", y = "% of Games with 2+ Plus Minus")
...
lm_fga <- lm(s_res ~ fga, data = log_sim)
bt_fga <- coef(lm_fga)
summary(lm_fga)
ggplot(log_sim, aes(fga, s_res)) + geom_point() +
  geom_abline(intercept = bt_fga[1], slope = bt_fga[2], color = "red") +
  labs(x = "Field Goal Attempts per Game", y = "Streakiness (Residual)")

log_model_fga <- glm(streaky ~ fga, data = log_sim, family = binomial)
beta_fga <- coef(log_model_fga)
grid_fga <- log_sim %>% data_grid(fga) %>%
    add_predictions(log_model_fga, type = "response")
summary(log_model_fga)
ggplot(log_sim, aes(fga)) + geom_point(aes(y = streaky)) +
  geom_line(aes(y = pred), data = grid_fga, color = "red", size = 1) +
  labs(x = "Field Goal Attempts per Game", y = "Streakiness (Binomial)")
```
