...
model_21 <- lm(games_above ~ ops, data = sim_21)
(beta <- coef(model_21))
ggplot(sim_21, aes(ops, games_above)) + geom_point() +
...
bat_data_day_21 %>% filter(player == "Rafael Devers") %>%
  group_by(game_date) %>%
  mutate(obp_game = stat_fct(on_base, pa),
         slg_game = stat_fct(tb, ab),
         ops_game = obp_game + slg_game,
...
bat_data_day_21 %>% filter(player == "Joey Votto") %>%
  group_by(game_date) %>%
  mutate(obp_game = stat_fct(on_base, pa),
         slg_game = stat_fct(tb, ab),
         ops_game = obp_game + slg_game,
